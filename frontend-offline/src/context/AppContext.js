...
export const AppProvider = ({ children }) => {
    const [theme, setTheme] = useState('aaa'); // default theme
    ...
    // fetch all data 
    useEffect(() => {
        const apiUrl = `${process.env.PUBLIC_URL}/data/general.json`;
        fetch(apiUrl)
            .then(response => {
                if (!response.ok) {
        ...
        const hotelURL = `${process.env.PUBLIC_URL}/data/hotel.json`;
        fetch(hotelURL)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
            ...
            .then(data => {
                setTheme(data["theme"])
                data.landing.private_hotel_slides.unshift("jbg.jpg");
                setHotelData(data);
            })
