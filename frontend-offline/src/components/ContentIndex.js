...
const ContentIndex = ({ globalId, setGlobalId }) => {
    ...
    useEffect(() => {
        ...
        if (globalId.substring(2, 8) === '000000') {
            ...
            setDisplayMap(tempData.map_urls ? `${process.env.PUBLIC_URL}/images/general/${tempData.map_urls[0].private_url}` : "")
            ...
            <div className='content-list' style={{ flexDirection: 'column' }}>
                {displayList && displayType && displayType === "overlay" && displayList.map((item, index) => (
                    <div key={index} style={{ position: 'relative' }} onClick={() => { setGlobalId(item.global_id) }}>
                        {item.private_image_url &&
                            <img style={{ width: '100%', height: '17.5rem' }} src={`${process.env.PUBLIC_URL}/images/general/${item.private_image_url}`} alt={item.name} />}
                        {item.private_brand_url &&
                            <img style={{ width: '100%', height: '17.5rem' }} src={`${process.env.PUBLIC_URL}/images/general/${item.private_brand_url}`} alt={item.name} />}
                ...
                {displayList && displayType && displayType === "sidebar" && displayList.map((item, index) => (
                    <div key={index} onClick={() => { setGlobalId(item.global_id) }} className='sidebar-view'>
                        {item.private_brand_url &&
                            <img src={`${process.env.PUBLIC_URL}/images/general/${item.private_brand_url}`} alt={item.name} style={{ width: '13rem', height: "8.625rem" }} />}
                        <span>
