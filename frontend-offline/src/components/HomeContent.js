...
const HomeContent = () => {
    ...
    useEffect(() => {
        const imageTimer = setTimeout(() => {
            setCurrentImageIndex((prevIndex) => (prevIndex + 1) % hotelData["landing"]["private_hotel_slides"].length);
    ...
    useEffect(() => {
        ...
        if (videoElement) {
            videoElement.src = `${process.env.PUBLIC_URL}/videos/${hotelData["landing"]["private_hotel_videos"][currentVideoIndex]}`;
        ...
        const handleVideoEnd = () => {
            if (!isCancelled) {
                setCurrentVideoIndex((prevIndex) => (prevIndex + 1) % hotelData["landing"]["private_hotel_videos"].length);
            }
        };
    ...
    return (
        <div className="main-content" >
            ...
            <div className="carousel" style={{ height: '19.5rem', overflow: 'hidden' }} onClick={() => {
                ...
                {hotelData && hotelData["landing"]["private_hotel_slides"].map((img, index) => (
                    <img key={index} style={getSlideStyles(index)}
                        src={`${process.env.PUBLIC_URL}/images/hotel/${img}`}
                        alt={index}
                    />
                ))}
            </div>
            ...
            {hotelData &&
                <div className="video-player" >
                    ...
                    >
                        <source
                            src={`${process.env.PUBLIC_URL}/videos/${hotelData["landing"]["private_hotel_videos"][currentVideoIndex]}`}
                            type="video/mp4"
                        />
