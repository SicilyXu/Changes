            ...
            {content && !content.custom_actions.includes("map-only") && (
                <div className='d-flex flex-column h-100'>
                    <div className='content-image'>
                        {content.private_image_urls.length > 1 ?
                            <Carousel interval={5000} pause={false}>
                                {content.private_image_urls.map((url, index) => (
                            ...
                            <img
                                ...
                                src={`${process.env.PUBLIC_URL}/images/general/${content.private_image_urls[0]}`}
                                alt={`${content.name}`}
                            />
                        }
                    </div>
                    {content.brand_url && !(content.custom_actions.includes("no-brand-picture")) ?
                        <div className='content-title'>

                            <span style={{ height: 0 }}>
                                <img src={`${process.env.PUBLIC_URL}/images/general/${content.brand_url}`} alt={content} style={{ width: '12.5rem', height: '9.375rem', transform: "translateY(-7.5rem)", border: "0.3rem solid white", objectFit: 'fill' }} />
                            </span>
                    ...
                    <div className='content-list'>
                        ...
                        <div className='content-content'>
                            ...
                            {content.map_urls && content.map_urls.length > 0 && content.map_urls.map((map, index) => (
                                <button className='btn-map' onClick={() => handleMapClick(map.name ? map.name : content.name + " map", map.private_url)} key={index}> {map.name ? map.name : "See Map"}</button>
                            ))}
                    ...
                    >
                        <div>
                            ...
                            <div className='map-content' style={{
                                ...
                                <img
                                    src={`${process.env.PUBLIC_URL}/images/general/${content.map_urls[0].private_url}`}
                                    alt="Map"
                                    style={{
                                        width: '62.5rem',
                                        height: '48.94rem',
