...
export const AppProvider = ({ children }) => {
    ...
    const login = async (email, password) => {
        const endpoints = ['hotel', 'advertiser']; // 包含两个尝试的 endpoint
        for (const endpoint of endpoints) {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/login`, {
                    ...
                    body: JSON.stringify({ endpoint, email, password }),
                });
                ...
                if (response.ok) {
                    ...
                    return {'status':true,'endpoint':endpoint}
                } else {
                    console.log(`Login failed for endpoint: ${endpoint}`);
                    if (endpoint === endpoints[endpoints.length - 1]) {
                        return {'status':false,'endpoint':undefined}
                    }
                }
            } catch (error) {
                console.error(`Error during login for endpoint: ${endpoint}:`, error);
                if (endpoint === endpoints[endpoints.length - 1]) {
                    return {'status':false,'endpoint':undefined};
                }
            }
